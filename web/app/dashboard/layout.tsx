    ...
    <div className="bg-milk min-h-screen text-dark-green">
      ...
      <aside className="fixed top-0 left-0 z-40 w-64 h-screen pt-24 bg-white border-r border-dark-green/5 transition-transform -translate-x-full sm:translate-x-0">
        <div className="h-full px-4 pb-4 overflow-y-auto flex flex-col">
          <div className="mb-8">
            <Link href={isConnected ? "/dashboard/buat" : "#"} onClick={() => !isConnected && alert("Hubungkan wallet kamu dulu, Capt!")} className={`flex items-center justify-center gap-3 w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95 ${isConnected ? "bg-dark-green text-milk shadow-dark-green/10 hover:-translate-y-1" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>
