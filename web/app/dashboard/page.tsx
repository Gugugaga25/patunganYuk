...
export default function DashboardHome() {
  ...
  if (loading) {
    return <div className="text-center p-20 font-black uppercase animate-pulse">Memuat Data Patungan...</div>;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {patunganList.length > 0 ? (
          patunganList.map((item) => (
            <div key={item.id} className="bg-white rounded-[2rem] border border-dark-green/5 shadow-sm hover:shadow-xl transition-all p-6 group">
              <div className="flex justify-between items-start mb-6">
                ...
                <span className="text-dark-green/20 text-[10px] font-black uppercase">#{item.id}</span>
              </div>

              <h3 className="text-xl font-black text-dark-green uppercase tracking-tighter mb-4 group-hover:text-accent-green transition-colors">{item.title}</h3>
              ...
              <div className="space-y-4 mb-8">
                <div>
                  ...
                  <div className="w-full h-2 bg-milk rounded-full overflow-hidden border border-dark-green/5">
                    <div className="h-full bg-accent-green transition-all duration-1000" style={{ width: `${Math.min((item.current_amount / item.target_amount) * 100, 100)}%` }}></div>
                  </div>
                </div>
              </div>
          ...
          <div className="col-span-full py-20 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-dark-green/10">
            ...
            <Link href="/dashboard/buat" className="mt-4 inline-block text-accent-green font-black uppercase text-[10px]">
              Buat Sekarang →
            </Link>
          </div>
        )}
